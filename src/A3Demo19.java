...
public class A3Demo19 {
	...
	public static void drawHill() { //This image is stationary and does not change
		...
		//Processing.size(800, 500); Calling size inside animation is a BAD idea

	}
	public static void drawStar(float localX, float localY, float chosenSize) { 
		//The "for" loop in this function serves to draw the saturation line repeatedly outward.
		...
		for (double circleWidth = 1; circleWidth < chosenSize; circleWidth=circleWidth+2) {
			...
			Processing.ellipse(localX, localY, chosenSize-circleWidth, chosenSize-circleWidth);
		}
	}
	...
	public static void drawCloud() { 
		//the for loop limits it to before the moon so that it doesn't go behind the moon.
		//I use focusAlpha as a focus point to bounce back and forth
		//Also! There is a key press function "p" stops the cloud moving and "i" starts the cloud moving.
		...
		if ((focusAlpha> Processing.width-250) || (focusAlpha<Processing.width-660)) {
			counter = counter *-1;
	...
	public static void drawHouses() {
		//These are a nested loop with a mousePress function in them-- the lights turn on and off
		houseX = startX;
		roofHeight = 10;
		roofBaseY = 416;
		startX = 450;
