...
public class A3Demo17 {
	...
	public static void draw() {
		if (isDay) {
			//in the day scence we have the sun and the fox appearing to day light
			Processing.background(184, 236, 255);
			Processing.fill(50, 100, 30);
			Processing.rect(0, 0.7 * Processing.height, Processing.width, Processing.height);
			...
			//in the night scene we have the moon going around but no foxes foxes are sleeping but later when we mouse press the day comes back and the sun and fox reappear
		...
		if (200+sunCount-sunSize/2> Processing.width && isDay) {
			//this next code helps define when the sun disappears completely then the next scene appears hence the radius is smaller than the width
			isDay=false;
		} else if ((!isDay) && (-moonSize/2+moonCount> Processing.width) ) {
			isNightOver = true;
		} else {
			//Same here for the moon we want to make sure the transition only happens when the whole moon has disappeared
	...
	public static void drawFox(double centerX, double centerY, double objWidth, double objHeight) {
		...
		Processing.fill(0);
		Processing.ellipse(headX+headSize/6, headY-headSize/6, headSize/4, headSize/4);
		Processing.fill(255);
