...
public class A3Demo20 {
	...
	public static void draw() {
		Processing.background(250, 237, 141);
		  ...
		  while (offsetX<Processing.width) {
		    ...
		    offsetX= offsetX+85;
		    if(discoCircles){
		      Processing.fill(Processing.random(128,255),Processing.random(128,255),Processing.random(128,255));
		    }else{
	...
	public static void drawCircles(int circleX, int circleY, int circleWidth, int circleLength) {
		if (discoCircles)
		    Processing.fill(Processing.random(128,255),Processing.random(128,255),Processing.random(128,255));
		  else
		    Processing.fill(247, 115, 161);
	...
	public static void drawButterfly(double localX, double localY, double objSize) {
		  double bigWingSize=objSize/4.0;

		  Processing.fill(0);
		  Processing.ellipse(localX+0.06*objSize, localY, bigWingSize, bigWingSize*1.1);

		  Processing.fill(250, 141, 219);
		  Processing.ellipse(localX, localY, bigWingSize, 1.2*bigWingSize);
		  Processing.ellipse(localX-0.1*objSize, localY, bigWingSize, 1.2*bigWingSize);

		  Processing.fill(0);
		  Processing.ellipse(localX, localY+bigWingSize/2, objSize/2, bigWingSize/3);

		  Processing.line(localX+bigWingSize, localY+bigWingSize/2, localX+objSize/3, localY+bigWingSize/4);
		  Processing.line(localX+bigWingSize, localY+bigWingSize/2, localX+objSize/3, localY+bigWingSize/2);
